...
# モード設定
MODE_CONFIG = {
    1: {'name': 'Lucky', 'notice_prob': 0.1},
    2: {'name': 'Normal', 'notice_prob': 0.1},
    3: {'name': 'Tight', 'notice_prob': 0.1}
}
...
def get_current_mode(total_in, total_out):
    if total_in == 0:
        return 2
...
def check_win(grid, is_free_game=False):
    ...
    for line_idx, line in enumerate(PAYLINES):
        ...
        if best_mult > 0:
            if line_idx >= 9:  # 縦ライン = フルベット
                line_win = best_mult * BET_AMOUNT
            else:  # 横/斜め = 1/9ベット
                line_win = best_mult * std_bet_per_line
            total_win += line_win
...
for i in range(SIMULATIONS):
    if i % 100 == 0:
        current_mode = get_current_mode(total_in, total_out)
    ...
    if bonus_symbols >= 3:
        bonus_count += 1
        sushi_win = simulate_sushi_bonus()
